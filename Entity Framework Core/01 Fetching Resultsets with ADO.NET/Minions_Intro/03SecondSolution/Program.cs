        ...
        static void Main(string[] args)
        {
            ...
            using (dbCon)
            {
                ...
                command = new SqlCommand(
                    $"SELECT m.Name, m.Age, ROW_NUMBER() OVER (ORDER BY m.Name) as RowNum FROM MinionsVillains AS mv JOIN Minions As m ON mv.MinionId = m.Id WHERE mv.VillainId = {villainId} ORDER BY m.Name",
                    dbCon);
